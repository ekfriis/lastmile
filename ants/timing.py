...
def _ensure_reftime(func):
   ''' Ensure times passed are in custom ref fmt '''
...
class TimePreferences(object):
   ...
   def random(self):
      #time_window = pymc.distributions.rcategorical(self.prefs)
   ...
   def satisfaction_probability(self, arrival_time):
      return sum( pref for pref, window in self.windows_and_prefs if
